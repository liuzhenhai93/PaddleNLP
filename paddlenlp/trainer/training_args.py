...
class TrainingArguments:
    ...
    def __post_init__(self):
        ...
        if self.amp_master_grad:
            if self.pipeline_parallel_degree <= 1 and self.tensor_parallel_degree <= 1 and (not self.sharding):
                raise ValueError(
                    "Temporarily amp master grad only support for tensor/pipeline/sharding"
                    " (stage 1、 stage 2、 stage3) parallel. Please set amp_master_grad to False."
                )
